...
export class ImageDescriptionController {
    ...
    public describeImage = async (req: Request, res: Response): Promise<void> => {
        try {
            ...
            // Handle dtype if provided in the body (REMOVED - dtype not used with llama.cpp server)
            // if (req.body && req.body.dtype) { ... }
        ...
        try {
            ...
            res.status(200).json({
                ...
                data: {
                    models,
                    default: {
                        // Provide the default model from the service, which reads from .env
